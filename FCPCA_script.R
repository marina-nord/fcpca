...
for (t in 1:3) {
  for (y in 1:9) {
    ...
    glm_res <- glm (q ~ ln_gdppc + gdppcgr + vdem_index + oilgas + duration + prevrc + party + military + personal + i, family=binomial(link="probit"), data=fcpca_data)
    names(glm_res$coefficients)[11]=names(fcpca_data[crisis_list[y]])
    assign(z, glm_res)
    w=paste0("model_",t+3,y)
    glm_res2 <- glm (q ~ ln_gdppc + vdem_index + oilgas + duration + prevrc + party + military + personal + i, family=binomial(link="probit"), data=fcpca_data)
    names(glm_res2$coefficients)[10]=names(fcpca_data[crisis_list[y]])
    assign(w, glm_res2)
  }
}
#
# saving results
{
  for (t in 1:3) {
    for (y in 1:9) {
      ...
      m=paste0('model_',t,y)
      mm=get(m)
      xx[[1]]=c(summary(mm)$coefficients[11], summary(mm)$coefficients[11,4], summary(mm)$coefficients[11,2])
      assign(xxx,xx)
      m=paste0('model_',t+3,y)
      mm=get(m)
      xx[[2]]=c(summary(mm)$coefficients[10], summary(mm)$coefficients[10,4], summary(mm)$coefficients[10,2])
      assign(xxx,xx)
    }
  }
}
...
#
model_14 <- glm (democratization ~ ln_gdppc + gdppcgr + vdem_index + oilgas + duration + prevrc + party + military + personal + default_dom, family=binomial(link="probit"), data=fcpca_data)
model_24 <- glm (democratization ~ ln_gdppc + gdppcgr + vdem_index + oilgas + duration + prevrc + party + military + personal + default_ext, family=binomial(link="probit"), data=fcpca_data)
#
...
# twin_bd_b (49)
#
model_17 <- glm (democratization ~ ln_gdppc + gdppcgr + vdem_index + oilgas + duration + prevrc + party + military + personal + bd_twin, family=binomial(link="probit"), data=fcpca_data)
model_27 <- glm (aa_transition ~ ln_gdppc + gdppcgr + vdem_index + oilgas + duration + prevrc + party + military + personal + bd_twin, family=binomial(link="probit"), data=fcpca_data)
#
# pooled probit models for transitions to democracy/autocracy and twin debt and banking crises
# twin_bd_d (50)
#
model_37 <- glm (democratization ~ ln_gdppc + gdppcgr + vdem_index + oilgas + duration + prevrc + party + military + personal + db_twin, family=binomial(link="probit"), data=fcpca_data)
model_47 <- glm (aa_transition ~ ln_gdppc + gdppcgr + vdem_index + oilgas + duration + prevrc + party + military + personal + db_twin, family=binomial(link="probit"), data=fcpca_data)
#
# creating figure 2 
#
library("jtools")
...
for (t in 1:6) {
  for (y in 1:9) {
    ...
    glm_res <- pglm (q ~ ln_gdppc + gdppcgr + vdem_index + oilgas + duration + prevrc + party + military + personal + i, family=binomial(link="probit"), model="random", index=c("cname", "year"), data=mydata)
    names(glm_res$estimate)[11]=names(fcpca_data[crisis_list[y]])
    assign(z, glm_res)
  }
}
#
...
#
{
  for (t in 1:6) {
    for (y in 1:9) {
      ...
      m=paste0('model_',t,y)
      mm=get(m)
      xx[[3]]=c(summary(mm)$estimate[11], summary(mm)$estimate[11,4], summary(mm)$estimate[11,2])
      assign(xxx,xx)
    }
  }
}
...
for (t in 1:6) {
  for (y in 1:9) {
    ...
    mydata$i <- getElement(fcpca_data, names(fcpca_data[crisis_list[y]]))
    print(names(fcpca_data[crisis_list[y]]))
    glm_res <- pglm (q ~ ln_gdppc + gdppcgr + vdem_index + oilgas + duration + prevrc + party + military + personal + southeastasia + mideast + africa + latam + exussr + i, family=binomial(link="probit"), model="random", index=c("cname", "year"), data=mydata)
    names(glm_res$estimate)[16]=names(fcpca_data[crisis_list[y]])
    assign(z, glm_res)
  }
}
#
...
#
{
  for (t in 1:6) {
    for (y in 1:9) {
      ...
      m=paste0('model_',t,y)
      mm=get(m)
      xx[[4]]=c(summary(mm)$estimate[16], summary(mm)$estimate[16,4], summary(mm)$estimate[16,2])
      assign(xxx,xx)
    }
  }
}
...
for (t in 1:6) {
  for (y in 1:9) {
    ...
    q=getElement(fcpca_data, names(fcpca_data[turnover_list[t]]))
    i=getElement(fcpca_data, names(fcpca_data[crisis_list[y]]))
    c_res <- clogit (q ~ ln_gdppc + gdppcgr + oilgas + duration + prevrc + party + military + personal + i + strata(ccode), data=fcpca_data, method=c("efron"))
    names(c_res$coefficients)[9]=names(fcpca_data[crisis_list[y]])
    assign(z, c_res)
  }
}
#
...
#
{
  for (t in 1:6) {
    for (y in 1:9) {
      ...
      m=paste0('model_',t,y)
      mm=get(m)
      xx[[5]]=c(summary(mm)$coefficients[9], summary(mm)$coefficients[9,5], summary(mm)$coefficients[9,3])
      assign(xxx,xx)
    }
  }
}
